  ...
  return createPortal(
    <>
      <div className="fixed inset-0 z-40 flex justify-end">
        {/* Overlay */}
        <div
          className="flex-1 bg-stone-950/40 backdrop-blur-sm absolute inset-0"
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-cream-300 flex-shrink-0">
            <div>
              <p className="text-xs tracking-widest2 uppercase font-sans text-stone-500">Кошик</p>
              <p className="font-display text-xl font-medium text-stone-950 mt-0.5">{totalCount} позицій</p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4">
            {items.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full gap-3 py-16">
                ...
                <p className="text-sm text-stone-400 font-sans">Кошик порожній</p>
              </div>
            ) : (
                        ...
                        <div className="flex items-center justify-between mt-3">
                          ...
                          <div className="flex items-center border border-cream-300">
                            <button
                              ...
                              className="w-7 h-7 flex items-center justify-center text-stone-500 hover:text-stone-950 hover:bg-cream-200 transition-colors text-lg leading-none"
                            >−</button>
                            <span className="w-7 text-center text-xs text-stone-700">{qty}</span>
                            <button
                              onClick={() => updateQty(product.id, qty + 1)}
                              className="w-7 h-7 flex items-center justify-center text-stone-500 hover:text-stone-950 hover:bg-cream-200 transition-colors"
                            >+</button>
                          </div>
                          {/* Remove */}
                          <button
