    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-screen-xl mx-auto px-6 md:px-12 py-4">
        <div className="flex items-center gap-2 text-xs text-stone-400 font-sans tracking-wide">
          <button onClick={onBack} className="hover:text-stone-700 transition-colors">
            {catName}
          </button>
          <span>/</span>
      ...
      <div className="max-w-screen-xl mx-auto px-6 md:px-12 pb-20">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16">
          ...
          <div>
            {/* Головне фото */}
            <div
              className="relative bg-cream-200 overflow-hidden cursor-zoom-in mb-3"
              ...
              {images.length > 1 && (
                <>
                  <button onClick={e => { e.stopPropagation(); setActive(i => (i - 1 + images.length) % images.length) }}
                    className="absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 bg-cream-100/90 hover:bg-cream-100 flex items-center justify-center text-stone-600 hover:text-stone-950 transition-colors text-xl shadow-sm">‹</button>
                  <button onClick={e => { e.stopPropagation(); setActive(i => (i + 1) % images.length) }}
                    className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 bg-cream-100/90 hover:bg-cream-100 flex items-center justify-center text-stone-600 hover:text-stone-950 transition-colors text-xl shadow-sm">›</button>
                </>
              )}
              {images.length > 0 && (
                <span className="absolute bottom-3 right-3 bg-cream-100/80 px-2 py-1 text-xs text-stone-500">🔍</span>
              )}
            </div>
          ...
          <div className="flex flex-col">
            ...
            <h1 className="font-display text-3xl md:text-4xl font-medium text-stone-950 leading-tight mb-3">
              {name}
            </h1>

            {product.price && (
              <p className="font-display text-3xl font-medium text-stone-950 mb-6">
                {product.price}
              </p>
            )}
