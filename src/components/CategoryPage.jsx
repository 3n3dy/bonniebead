  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="max-w-screen-xl mx-auto px-6 md:px-12 py-14 md:py-20">
          ...
          <div className="grid grid-cols-12 items-end gap-6">
            <div className="col-span-12 md:col-span-7">
              ...
              <h2 className="font-display text-5xl md:text-7xl font-medium leading-tight">{category.name}</h2>
            </div>
            <div className="col-span-12 md:col-span-5 md:text-right">
              <p className="font-sans text-sm text-cream-300 leading-relaxed">{category.note}</p>
              <p className="index-label mt-3 text-stone-500">{category.products.length} найменувань</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {category.products.map((product, i) => (
              <div key={product.id} className="group/card" style={{ animationDelay: `${i * 55}ms` }}>
                <ProductCard product={product} catName={category.name} />
              </div>
            ))}
          </div>
        )}
      ...
      <div className="max-w-screen-xl mx-auto px-6 md:px-12 py-8 flex items-center justify-between">
        ...
        <p className="index-label">{category.name} / BONNIEBEAD</p>
      </div>
    </div>
  )
}
